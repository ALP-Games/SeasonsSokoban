...
func ready(world: GameWorld) -> void:
	...
	for entity: GridEntity in get_children():
		...
		entity.set_entity_container(self)
		# map_pos should be saved to a map for entities
		# and it should be saved for the entity also
		var global_pos := _game_world.to_global(_game_world.map_to_local(map_pos))
		entity.global_position = global_pos


func move_entity(entity: GridEntity, position: Vector2i) -> Vector2:
	return _game_world.to_global(_game_world.map_to_local(position))
